...
class Test_Login_QABrains:
    ExcelFile_Path = "C:\\Users\\aksha\\PycharmProjects\\PythonProject5\\testcases\\Test_Data\\testCase_data.XLSX"
    ...
    def test_login_QAbrains_TC01(self, setup):
        ...
        rows = ExcelMethods.getRowNumber(self.ExcelFile_Path, "Sheet1")

        TestCase_Status_List = []
        for r in range(2, rows + 1):
            self.driver.get("https://practice.qabrains.com/")

            self.log.info("Reading Email for row = " + str(r))
            email = ExcelMethods.readData(self.ExcelFile_Path, "Sheet1", r, 2)
            self.log.info("Reading Password for row = " + str(r))
            password = ExcelMethods.readData(self.ExcelFile_Path, "Sheet1", r, 3)
            self.log.info("Entering Email for row = " + str(r))
            ...
            Expected_Result = ExcelMethods.readData(self.ExcelFile_Path, "Sheet1", r, 4)
            Actual_Result = lp.Validate_LoginStatus()

            if Expected_Result == "Pass" and Actual_Result == "Pass":
                self.log.info("Expected Result & Actual Result both are Pass for row = " + str(r))
                TestCase_Status_List.append("PASS")
                self.log.info("Updated TestCase_Status_List as " + str(TestCase_Status_List))
                self.log.info(
                    "Updating Actual Result value as " + str(
                        Actual_Result) + " in testCase_data.XLSX Excel File for row = " + str(r))
                ExcelMethods.writeData(self.ExcelFile_Path, "Sheet1", r, 5, "Pass")
                self.log.info("Taking Screenshot for row = " + str(r))
                self.driver.save_screenshot(
                    f"C:\\Users\\aksha\\PycharmProjects\\PythonProject5\\Screenshots\\test_login_QAbrains_TC01_PASS_for_row_{r}_.png")
                self.log.info("Taking Allure Screenshot for row = " + str(r))
                ...
                self.log.info("Expected Result & Actual Result both are Fail for row = " + str(r))
                TestCase_Status_List.append("PASS")
                self.log.info("Updated TestCase_Status_List as " + str(TestCase_Status_List) + " for row = " + str(r))
                self.log.info(
                    "Updating Actual Result value as " + str(
                        Actual_Result) + " in testCase_data.XLSX Excel File for row = " + str(r))
                ExcelMethods.writeData(self.ExcelFile_Path, "Sheet1", r, 5, Actual_Result)
                self.log.info("Taking Screenshot for row = " + str(r))
                self.driver.save_screenshot(
                    f"C:\\Users\\aksha\\PycharmProjects\\PythonProject5\\Screenshots\\test_login_QAbrains_TC01_Fail_for_row_{r}_.png")
                self.log.info("Taking Allure Screenshot for row = " + str(r))
                ...
                self.log.info("Expected Result: " + str(Expected_Result) + "  " + "Actual Result: " + str(
                    Actual_Result) + " Not Same for row = " + str(r))
